...
# local modules
from housing_cont_model import ConsumerProblem, Operator_Factory
...
if __name__ == "__main__":
	...
	for plot_t in list(range(term_t, cp.T)):
		...
		for i_z in [1]:
			for col_ih, i_h, lbs in zip([0,1,2], [200,350,400], labs):
				...
				ax_pol[1].plot(x1, g_1,color=colors[col_ih],\
													label = lbs,\
													linewidth=0.75)
				ax_pol[0].plot(x2,g_2,color=colors[col_ih],\
													label = lbs,\
													linewidth=0.75)
				...
				ax_pol_a[1].plot(x1a, g_1a,color=colors[col_ih],\
													label = lbs,\
													linewidth=0.75)
				ax_pol_a[0].plot(x2a,g_2a,color=colors[col_ih],\
													label = lbs,\
													linewidth=0.75)
				...
				ax_pol[0].set_title("VFI", fontsize=11)
				ax_pol[1].set_title("FUES-EGM", fontsize=11)
