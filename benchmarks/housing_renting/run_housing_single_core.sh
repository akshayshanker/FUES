...
#PBS -r y
#PBS -o logs/
# Note: Create logs/ folder before submitting: mkdir -p benchmarks/housing_renting/logs
...
# PBS_O_WORKDIR is the directory where qsub was invoked (benchmarks/housing_renting/)
...
for CONFIG_NAME in "${CONFIG_TO_RUN[@]}"; do
    ...
    # All logs go to benchmarks/housing_renting/logs/ (same as PBS -o)
