...
matching the README and the [Quickstart](../start-here/quickstart.md):
the library alone (Option 1), or the full environment for the example
...
(`.venv`; `~/venvs/fues` on Gadi), installs everything listed in the next
subsection, verifies the imports, and activates the environment in your
current shell; on later runs it only activates. Pass `--update` to
`git pull` and reinstall after new commits.
...
The `[examples]` extra adds the application dependencies (`kikku`, pinned
at a tagged release, plus plotting and estimation packages) on top of the
core install. The last two lines install the pinned dolo-plus compiler
that the example models import; they are deliberately `--no-deps` — the
forks' packaging metadata conflicts with this repo's pins — which is why
the extra cannot pull them in.
...
With the examples environment, the quick test suite gives a deeper check
in about a second:
