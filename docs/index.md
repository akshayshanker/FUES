...
work well. Setting `endog_mbar=True` lets FUES compute a grid-local threshold
from `del_x_cntn`.

To compare FUES against MSS, RFC, and LTM on the same problem, use the unified
`EGM_UE` interface:
...
and `RFC` do not. See the [durables
application](examples/continuous_housing_model.md) for the main non-monotone
benchmark.
