...
def _make_egm_adjuster(callables, grids, stage):
    ...
    a_include_inter = bool(int(sett.get("adj_include_intersections", 0)))
    ...
    # --- Precompute vectorized prescan constants ----------------------
    _use_vectorized = bool(sett.get("vectorized_prescan", False))
    _h_arr = np.asarray(h_choice_grid)

    if _use_vectorized and not _cd_utility:
        # Separable: du_h depends only on h, not c
        _s_alpha = float(cal["alpha"])
        _s_gc = float(cal["gamma_c"])
        _s_gh = float(cal["gamma_h"])
        _s_kappa = float(cal["kappa"])
        _s_chi = float(cal.get("chi", 0.0))
        _s_beta = float(cal["beta"])
        _s_threshold = (
            (1.0 - _s_alpha)
            * _s_kappa ** (1.0 - _s_gh)
            * _h_arr ** (-_s_gh)
    ...
    if _use_vectorized and _cd_utility:
        # Cobb-Douglas: du_h depends on both c and h
        _cd_alpha = float(cal["alpha"])
        _cd_rho = float(cal["rho"])
        _cd_d_ubar = float(cal["d_ubar"])
        _cd_beta = float(cal["beta"])
        _cd_htot = _h_arr + _cd_d_ubar          # (n_he,)
        # Exponents used in Euler inversion and du_h
        _cd_c_power = _cd_alpha * (1.0 - _cd_rho) - 1.0
        _cd_d_power = (1.0 - _cd_alpha) * (1.0 - _cd_rho)
        _cd_inv_denom = _cd_alpha * _cd_htot ** _cd_d_power  # (n_he,)
        _cd_duh_c_pow = _cd_alpha * (1.0 - _cd_rho)
        _cd_duh_h_pow = _cd_d_power - 1.0
    ...
    def inv_euler_h_residual(h_choice, dv_a_cntn, dv_h_cntn):
        ...
        #
        # We pre-compute c on the exogenous grid (exact, no interpolation)
        # and then interpolate c at the off-grid root.  This is more
        # accurate than interpolating dv_a (steep, ∝ c^{-γ_c}) and
        # inverting, because c is smoother and linear interpolation
        # error is proportional to the second derivative.
        c_cntn_on_grid = np.empty(n_a)
        for ia in range(n_a):
            c_cntn_on_grid[ia] = du_c_inv_fn(dv_a_cntn[ia], h_choice)

        m_cntn_raw = np.zeros(egm_n)
        for j in range(n_roots):
            a_p = a_nxt_cntn[j]
            if a_p > 0.0:
                c = interp_as_scalar(a_grid, c_cntn_on_grid, a_p)
    ...
    # ================================================================
    # Vectorized root pre-scan (separable utility only)
    #
    # Computes the housing FOC residual as a single numpy array op,
    # detects ALL sign changes via signbit+XOR, and recovers (c, m, V)
    # at each root from boundary values.  Returns the same 4-tuple as
    # cntn_to_dcsn_egm so fues_refine is unchanged.
    #
    # Gated on: _use_vectorized (separable + root_eps == 0).
    # CD utility falls back to the original njit loop.
    # ================================================================

    def _vectorized_prescan(d_aV, d_hV, V_cntn):
        n_z, n_a, n_he = d_aV.shape
        assert n_he == len(h_choice_grid), (
            f"h axis {n_he} != h_choice_grid {len(h_choice_grid)}")

        # --- Step 1: residual as array arithmetic ---
        dv_a_safe = np.maximum(d_aV, 1e-15)   # guard ≤0 for Euler inv
        if _cd_utility:
            # CD: c = (d_aV / (α·htot^((1-α)(1-ρ))))^(1/(α(1-ρ)-1))
            inv_d = _cd_inv_denom[np.newaxis, np.newaxis, :]   # (1,1,n_he)
            c_arr = (dv_a_safe / inv_d) ** (1.0 / _cd_c_power)
            htot = _cd_htot[np.newaxis, np.newaxis, :]
            du_h_arr = ((1.0 - _cd_alpha)
                        * c_arr ** _cd_duh_c_pow
                        * htot ** _cd_duh_h_pow)
            R = fac_housing * d_aV - d_hV - du_h_arr
        else:
            # Separable: du_h = (1-α)κ^(1-γ_h)·h^(-γ_h)
            R = (fac_housing * d_aV
                 - d_hV
                 - _s_threshold[np.newaxis, np.newaxis, :])

        # --- Step 2: sign-change detection ---
        sb = np.signbit(R)
        crossings = sb[:, :-1, :] ^ sb[:, 1:, :]
        iz_arr, ia_arr, ih_arr = np.where(crossings)
        n_cross = len(iz_arr)

        # --- Step 3: root interpolation (guarded) ---
        R_lo = R[iz_arr, ia_arr, ih_arr]
        R_hi = R[iz_arr, ia_arr + 1, ih_arr]
        denom = R_lo - R_hi
        safe = np.abs(denom) > 1e-30
        t = np.where(safe, R_lo / denom, 0.5)
        a_lo = a_grid[ia_arr]
        a_hi = a_grid[ia_arr + 1]
        a_root = a_lo + t * (a_hi - a_lo)

        # --- Step 4: recovery from boundary values ---
        # Use (1-t)*lo + t*hi (numerically stable when |lo| >> |hi|)
        # instead of lo + t*(hi - lo) which suffers catastrophic
        # cancellation when marginal values span many orders of magnitude.
        omt = 1.0 - t                           # one minus t

        dv_a_lo = d_aV[iz_arr, ia_arr, ih_arr]
        dv_a_hi = d_aV[iz_arr, ia_arr + 1, ih_arr]
        dv_a_root = np.maximum(omt * dv_a_lo + t * dv_a_hi, 1e-15)

        h_vals = _h_arr[ih_arr]

        # V_cntn at root
        v_lo = V_cntn[iz_arr, ia_arr, ih_arr]
        v_hi = V_cntn[iz_arr, ia_arr + 1, ih_arr]
        v_root = omt * v_lo + t * v_hi

        if _cd_utility:
            # CD Euler inv: c = (d_aV / (α·htot^d_power))^(1/c_power)
            htot_r = h_vals + _cd_d_ubar
            c_root = (dv_a_root
                      / (_cd_alpha * htot_r ** _cd_d_power)
                      ) ** (1.0 / _cd_c_power)
            # CD utility: (c^α · htot^(1-α))^(1-ρ) / (1-ρ)
            agg = c_root ** _cd_alpha * htot_r ** (1.0 - _cd_alpha)
            u_root = agg ** (1.0 - _cd_rho) / (1.0 - _cd_rho)
            V_root = u_root + _cd_beta * v_root
        else:
            # Separable Euler inv: c = (α / d_aV)^(1/γ_c)
            c_root = (_s_alpha / dv_a_root) ** (1.0 / _s_gc)
            # Separable utility
            c_util = _s_alpha * c_root ** (1.0 - _s_gc) / (1.0 - _s_gc)
            h_util = ((1.0 - _s_alpha)
                      * (_s_kappa * h_vals) ** (1.0 - _s_gh)
                      / (1.0 - _s_gh))
            V_root = c_util + h_util - _s_chi + _s_beta * v_root

        # Endogenous wealth: m = c + a' + (1+τ)h'
        m_root = c_root + a_root + fac_housing * h_vals

        # --- Step 5: scatter into (n_z, n_he, egm_n) arrays ---
        m_cntn_raw = np.zeros((n_z, n_he, egm_n))
        v_cntn_raw = np.zeros((n_z, n_he, egm_n))
        a_nxt_cntn = np.zeros((n_z, n_he, egm_n))
        h_choice_cntn = np.zeros((n_z, n_he, egm_n))
        root_count = np.zeros((n_z, n_he), dtype=np.intp)

        valid = a_root > 0.0
        iz_v, ih_v = iz_arr[valid], ih_arr[valid]
        a_v, m_v, V_v, h_v = (
            a_root[valid], m_root[valid], V_root[valid], h_vals[valid])

        for idx in range(len(iz_v)):
            i_z, i_h = int(iz_v[idx]), int(ih_v[idx])
            k = root_count[i_z, i_h]
            if k < egm_n - 1:           # reserve last slot for constraint
                a_nxt_cntn[i_z, i_h, k] = a_v[idx]
                m_cntn_raw[i_z, i_h, k] = m_v[idx]
                v_cntn_raw[i_z, i_h, k] = V_v[idx]
                h_choice_cntn[i_z, i_h, k] = h_v[idx]
                root_count[i_z, i_h] = k + 1

        # --- Step 6: borrowing constraint ---
        # Separable: closed-form.  CD: brentq over c.
        # Same logic as inv_euler_h_residual's constraint section.
        for ihp in range(n_he):
            h_ch = h_choice_grid[ihp]
            h_cost = fac_housing * h_ch
            for iz in range(n_z):
                dv_a_1d = d_aV[iz, :, ihp]
                dv_h_1d = d_hV[iz, :, ihp]

                nr = root_count[iz, ihp]
                has_b = False
                for k in range(nr):
                    if a_nxt_cntn[iz, ihp, k] == b:
                        has_b = True
                        break
                if has_b:
                    continue

                dv_h_b = interp_as_scalar(a_grid, dv_h_1d, b)
                ...
                if _cd_utility:
                    ...
                    du_h_val = du_h_fn(0.0, h_ch)
                    marg_arg = (dv_h_b + du_h_val) / fac_housing
                    if marg_arg > 0.0:
                        c_at_b = du_c_inv_fn(marg_arg, h_ch)

                if c_at_b > 0.0 and du_c_fn(c_at_b, h_ch) >= dv_a_b:
                    v_1d = V_cntn[iz, :, ihp]
                    v_at_b = interp_as_scalar(a_grid, v_1d, b)
                    a_nxt_cntn[iz, ihp, -1] = b
                    m_cntn_raw[iz, ihp, -1] = c_at_b + b + h_cost
                    v_cntn_raw[iz, ihp, -1] = (
                        u_fn(c_at_b, h_ch) + bellman_discount(v_at_b))
                    h_choice_cntn[iz, ihp, -1] = h_ch
    ...
    def dcsn_mover(vlu_cntn, grids):
        ...
        if _use_vectorized:
            ...
            # Fallback: root_eps > 0 uses coarse scan in original loop
            m_cntn_raw, v_cntn_raw, a_nxt_cntn, h_choice_cntn = \
                cntn_to_dcsn_egm(
                    vlu_cntn["d_aV"], vlu_cntn["d_hV"], vlu_cntn["V"])
