...
def _run_sweep_params_path(run, base_calib, base_config):
    ...
    for idx, point in enumerate(points):
        ...
        if point['method_overrides']:
            mo.update(point['method_overrides'])
        calib_use = {**base_calib, **point['calib_overrides']}
        cfg_use = {**base_config, **point['setting_overrides']}
        syntax = str(run.syntax_dir)

        if run.simulate:
            def trial(
                _method=method, _mo=mo, _calib=calib_use, _cfg=cfg_use,
                _syntax=syntax, _run=run,
            ):
                nest, grids = solve(
                    _syntax, method=_method,
                    method_overrides=_mo,
                    calib_overrides=_calib,
                    setting_overrides=_cfg,
                    verbose=False)
                adj0 = nest["periods"][0]["stages"]["adjuster_cons"]
                method_label = _method or read_scheme_method(
                    adj0, 'upper_envelope')
                st0 = nest["periods"][0]["stages"]["keeper_cons"]
                use_emp = (
                    st0.calibration.get("init_method", "standard") == "empirical")
                sim_data = simulate_lifecycle(
                    nest, grids,
                    N=_run.n_sim, seed=_run.seed,
                ...
                return {
                    ...
                    'euler_c_mean': ec_stats.get('combined', {}).get(
                        'mean', np.nan),
                    'euler_c_keeper': ec_stats.get('keeper', {}).get(
                        'mean', np.nan),
                    'euler_c_adjuster': ec_stats.get('adjuster', {}).get(
                        'mean', np.nan),
        ...
        primary_key = 'solve_ms'
        best_metrics = None
        best_primary = None
        for rep in range(run.sweep_runs):
            metrics = trial()
            primary_val = metrics[primary_key]
            print(f"  rep {rep + 1}/{run.sweep_runs}  "
                  f"{primary_key}={primary_val:.4g}")
            if best_primary is None or (
                    not math.isnan(primary_val) and
                    primary_val < best_primary):
                best_primary = primary_val
                best_metrics = metrics

        if best_metrics is None:
            nan_keys = (
                ['solve_ms', 'keeper_ms', 'adj_ms', 'method']
                + (['euler_c_mean', 'euler_c_keeper', 'euler_c_adjuster',
                    'euler_h_mean', 'adj_rate'] if run.simulate else []))
            best_metrics = {k: (float('nan') if k != 'method' else '')
                            for k in nan_keys}

        n_a_eff = _effective_setting('n_a', point, base_config)
        row = {**point['params'], **best_metrics, 'n_a': n_a_eff}
        results.append(row)
        ...
        summaries.append({
            ...
            'Avg_Keeper_ms': best_metrics['keeper_ms'],
            'Avg_Adj_ms': best_metrics['adj_ms'],
