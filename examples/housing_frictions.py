...
class ConsumerProblem:
	...
	def __init__(self, 
		...
		def y_func(t,xi):
			...
			wage_age = np.dot(np.array([1, t, np.power(t,2), np.power(t,3), \
						np.power(t,4)]).astype(np.float64), lambdas[0:5])
...
def Operator_Factory(cp):
	...
	def bellman_operator(t,V):
		...
		for state in range(len(X_all)):
			...
			a_prime_adj_star_1  = qe.optimize.nelder_mead(obj_noadj,\
														x0_nadj_2,\
														bnds_nadj_2,\
														args = args_nadj_1)[0][0]
	...
	def refine_no_adj(endog_grid_unrefined,\
		...
		for index_h_today in range(len(asset_grid_H)):
			for index_z in range(len(z_vals)):
				...
				new_a_prime_refined[index_z, :,index_h_today]\
					[np.where(new_a_prime_refined[index_z, :,index_h_today]<b)] = b
				...
				for i_a in range(len(asset_grid_A)):
					if asset_grid_A[i_a]<a_under_bar:
						...
						v_at_amin = \
								u(c,asset_grid_H[index_h_today]*(1-delta), 0)\
								 + beta*eval_linear(UGgrid_all,\
								 	 V_prime[index_z,:], point,xto.LINEAR)
			...
			for index_z in range(len(z_vals)):
				...
				for i in range(len(a_primes)):
					if a_primes[i]>0:
						...
						vf_unrefined[index_z, index_h_prime,i] = u(c_val,\
													h_prime, chi) + v_prime
	...
	def coleman_operator(t,V_prime,\
		...
		for state in range(len(X_all)):
			...
			v_nadj_val = interp_as(asset_grid_A, v_nadj[i_z, :, i_h], np.array([wealth_nadj]))[0]
			c_nadj_val = interp_as(asset_grid_A, c_nadj[i_z, :, i_h], np.array([wealth_nadj]))[0]
			a_prime_nadj_val = interp_as(asset_grid_A, a_prime_nadj[i_z, :, i_h], np.array([wealth_nadj]))[0]
