...
from fella import ConsumerProblem, Operator_Factory, iterate_euler
from fella import iterate_euler, welfare_loss_log_utility
...
def plot_pols_fella(results_FUES, results_DCEGM, cp):
    ...
    for i, col, lab in zip(H_inds, fues_colors, ['$H_{t}$ = low', '$H_{t}$ = med.', '$H_{t}$= high']):
        ...
        fues_masked = mask_jumps(results_FUES['state']['c'][1, :, i], threshold=10)
        dcegm_masked = mask_jumps(results_DCEGM['state']['c'][1, :, i], threshold=10)
    ...
    # Saving figures to local images directory
    fig_pol_fella.savefig('images/fella_policy_function.png')
...
if __name__ == "__main__":
    ...
    income_vals = np.array([0.25, 0.45, 0.70, 1.0, 1.40, 2.0, 3.0])
    ...
    Probs = np.array([
        ...
        [0.00, 0.00, 0.00, 0.00, 0.02, 0.18, 0.80]   # Highest income
    ])
    ...
    results_FUES_10 = iterate_euler(cp_10, method='FUES', max_iter=3, tol=1e-03, verbose=True)
    results_DCEGM_10 = iterate_euler(cp_10, method='DCEGM', max_iter=3, tol=1e-03, verbose=True)
    ...
    summarize_results(results_FUES_10, results_DCEGM_10, 5, z_series, cp_10)
