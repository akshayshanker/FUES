...
Three-stage branching period::
    ...
    labour_mkt_decision (branching, max)
      |-- work   -> work_cons   (EGM + upper envelope)
      +-- retire -> retire_cons (EGM, no upper envelope)
...
class RetirementModel:
    ...
    def from_period(cls, period, equations=None):
        ...
        Reads calibration from the ``work_cons`` stage's
        ``.calibration`` attribute and settings from its
        ``.settings`` attribute.
        ...
        return cls(
            ...
            smooth_sigma=float(cal['smooth_sigma']),
            y=float(cal['y']),
            b=float(cal['b']),
            grid_max_A=float(cal['grid_max_A']),
            grid_size=int(cal['grid_size']),
            T=int(cal.get('T', settings.get('T', 60))),
            m_bar=float(settings.get('m_bar', 1.2)),
            equations=equations,
        )
