...
def write_plot_outputs(
    ...
    *,
    ...
    if not solutions:
        return
    # Representative model for grids/settings: rows need not include the
    # default UE methods (e.g. a single --slot-spec row), so take the first
    # row present rather than indexing UE_METHODS[0].
    model = next(iter(solutions.values()))["model"]
...
def _print_method_table(solutions: dict[str, dict], model) -> None:
    """Console-print Method × (Euler, UE time, Total time).

    UE_METHODS rows first (paper order), then any other row labels
    (e.g. from custom --slot-spec runs) in insertion order.
    """
    print()
    print("| Method | Euler Error    | Avg UE time(ms) | Total time(ms) |")
    print("|--------|----------------|-----------------|----------------|")
    ordered = [*UE_METHODS, *[k for k in solutions if k not in UE_METHODS]]
    for m in ordered:
