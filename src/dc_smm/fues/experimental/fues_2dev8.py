"""Fast Upper‑Envelope Scan (FUES) - Optimized with Separate Functions
Performance optimized version that maintains separate backward/forward scan functions
for clarity while incorporating efficiency improvements.

Based on fues_2dev4.py with optimizations:
1. Conditional intersection array allocation (only when needed)
2. Reduced redundant computations
3. Optimized circular buffer access
4. Early exit conditions in scan functions
5. Pre-computed values to avoid repeated calculations
6. Simplified flag logic while keeping function separation
"""
...
def backward_scan_combined(m_buf, m_head, LB, e_grid, vf_full, a_prime, 
    ...
    - Early exit when no valid entries
    - Reduced conditional checks
    - Pre-computed loop limit
    ...
    # Optimization: limit search to valid entries
    search_limit = min(LB, m_head) if m_head > 0 else 0
    if search_limit == 0:
        return keep_j, m_ind
    
    # Search backwards for last same-branch point (most recent to oldest)
    for t in range(search_limit):
        ...
        if check_drop:
            ...
            # For intersection finding (check_drop=False)
...
def find_forward_same_branch(e_grid, a_prime, start_idx, j_idx, N, LB, m_bar):
    ...
    Optimized with early exit and bounds checking.
    
    Returns found flag and index.
    """
    scan_limit = min(LB, N - start_idx - 1)
    
    for f in range(scan_limit):
        idx = start_idx + 1 + f
        de = max(1e-200, e_grid[idx] - e_grid[j_idx])
        g_a = np.abs((a_prime[idx] - a_prime[j_idx]) / de)
        if g_a < m_bar:
            return True, idx
    return False, -1
...
def forward_scan_case_a(e_grid, vf_full, a_prime, i, j, N, LB, m_bar, g_1):
    ...
    scan_limit = min(LB, N - i - 2)
    
    for f in range(scan_limit):
        idx = i + 2 + f
        de = max(1e-200, e_grid[i+1] - e_grid[idx])
        g_f_a = np.abs((a_prime[j] - a_prime[idx]) / de)
        if g_f_a < m_bar:
            idx_f = idx
            # Compute g_f_vf for this point
            g_f_vf_at_idx = (vf_full[i+1] - vf_full[idx]) / de
...
# ---------------------------------------------------------------------
# Core scan - Optimized while keeping separate functions --------------
# ---------------------------------------------------------------------
...
def _scan(e_grid, vf, a_prime, policy_2, del_a,
    ...
    Optimized version with:
    1. Conditional intersection array allocation
    2. Reduced redundant computations
    3. Separate scan functions for clarity
    4. Optimized loop conditions
    5. Pre-computed values where possible
    """

    N = e_grid.size
    vf_full = vf.copy()
    
    # Arrays to track intersection points - only allocate if needed
    if include_intersections:
        ...
        # Dummy values when not tracking intersections
        n_inter = 0
        use_intersection_as_k = False
        added_intersection_last_iter = False
    ...
    for i in range(N - 2):
        ...
        if right_turn_jump:
            keep_i1 = False
            if fwd_scan_do:
                # Use the optimized forward scan function
            ...
            if keep_i1:
                ...
                if include_intersections and idx_f != -1:
                    ...
                    _, idx_b = backward_scan_combined(
                        ...
                        i, j, i+1, False, False, False, 0.0, m_bar, check_drop=False)
                    found_b = idx_b != -1
        ...
        # Use the optimized combined backward scan function with drop check
