...
def _scan(e_grid, vf, a_prime, del_a,
          ...
          endog_mbar, padding_mbar, add_kinks = True):
    ...
    N = e_grid.size
    vf_full = vf.copy()
    #cap       = e_grid.size + (e_grid.size // 4)    # 25 % spare
    #e_grid.resize(cap, refcheck=False)
    #vf.resize(cap, refcheck=False)
    #vf_full.resize(cap, refcheck=False)
    #a_prime.resize(cap, refcheck=False)
    #del_a.resize(cap, refcheck=False)
    #write_ptr = e_grid.size // 1   
    ...
    for i in range(N - 2):
        ...
        if not keep_j:
            ...
            if left_turn:
                ...
                # ---- (A) unconditional kink insertion on THIS kept left-turn ----
                if left_turn and m_ind != -1 and add_kinks:
                    p_in  = np.array([e_grid[k],             vf_full[k]])
                    p_kb = np.array([e_grid[idx_grad_base], vf_full[idx_grad_base]])
                    p_j   = np.array([e_grid[j],             vf_full[j]])
                    p_i1  = np.array([e_grid[i+1],           vf_full[i+1]])
                    kink  = seg_intersect(p_in, p_kb, p_j, p_i1)

                    vf_full[j] = kink[1]
                    vf[j]      = kink[1]      # keep vf array in sync
                    e_grid[j]  = kink[0]
                    # no write_ptr, no extra node
                # -----------------------------------------------------------------

                # ---- (B) preserve kink for PREVIOUS left-turn (consecutive) ----
                if last_turn_left:
                    p_k  = np.array([e_grid[k],             vf_full[k]])
                    p_kb = np.array([e_grid[idx_grad_base], vf_full[idx_grad_base]])
                    p_prev = np.array([e_grid[j],           vf_full[j]])
                    p_i1 = np.array([e_grid[i+1],           vf_full[i+1]])

                    intr = seg_intersect(p_k, p_kb, p_prev, p_i1)

                    vf[j]       = np.nan                    # retire old j
                    vf_full[j]  = intr[1]
                    vf[j]       = intr[1]                   # ★ set vf as well
                    e_grid[j]   = intr[0]
                # -----------------------------------------------------------------
